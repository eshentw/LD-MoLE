...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        A_w = torch.stack(
            [getattr(self, f"lora_A{i}").weight for i in range(self.lora_nums)], dim=0)  # [num_lora, r, in_features]
        B_w = torch.stack(
            [getattr(self, f"lora_B{i}").weight for i in range(self.lora_nums)], dim=0)  # [num_lora, out_features, r]
        
        # x_drop: [B, S, in_features] -> [B, S, 1, in_features] -> [B, S, num_lora, in_features]
        x_expanded = x_drop.unsqueeze(2).expand(-1, -1, self.lora_nums, -1)  # [B, S, num_lora, in_features]
        # [B, S, num_lora, in_features] @ [num_lora, in_features, r] -> [B, S, num_lora, r]
        lora_a_out = torch.einsum("bsni,nir->bsnr", x_expanded, A_w.transpose(-2, -1))
        # [B, S, num_lora, r] @ [num_lora, r, out_features] -> [B, S, num_lora, out_features]
        lora_out = torch.einsum("bsnr,nro->bsno", lora_a_out, B_w.transpose(-2, -1))

        gate_score, routing_weight, lam = None, None, None
        if x.isnan().any() or x.isinf().any():
                print("x is NaN or Inf, returning None", x)
        if self.lora_nums > 1:
            if self.sparsegen:
                gate_score = self.lora_route(x_drop)
            else:
                gate_score = self.lora_route(x)
            if gate_score.isnan().any() or gate_score.isinf().any():
                print("Gate score is NaN or Inf, returning None", gate_score)

            if self.sparsegen:
                # routing_weight, lam = self.sparsegen(gate_score, x)
                routing_weight, lam = self.sparsegen(gate_score, x_drop)
                routing_weight = routing_weight.to(base.dtype)
                if lam is not None:
                    lam = lam.to(base.dtype)
            else:
                routing_weight = torch.softmax(gate_score, dim=-1).to(base.dtype)
            
            # [batch, seq, num_lora, out] * [batch, seq, num_lora] -> [batch, seq, out]
            lora_result = torch.einsum("bsno,bsn->bso", lora_out, routing_weight)
            result = base + lora_result * self.scaling
        else:
            result = base + lora_out.squeeze(2) * self.scaling
